...
const getSecret = (type: "access_token" | "refresh_token" | "password_reset" | "email_verification") => {
  switch (type) {
    case "access_token":
      return new TextEncoder().encode(process.env.ACCESS_TOKEN_SECRET || "access-secret-change-me");
    case "refresh_token":
      return new TextEncoder().encode(process.env.REFRESH_TOKEN_SECRET || "refresh-secret-change-me");
    case "password_reset":
      return new TextEncoder().encode(process.env.PASSWORD_RESET_TOKEN_SECRET || "password-reset-secret");
    case "email_verification":
      return new TextEncoder().encode(process.env.EMAIL_VERIFICATION_TOKEN_SECRET || "email-verification-secret");
  }
};

const getExpiry = (type: "access_token" | "refresh_token" | "password_reset" | "email_verification") => {
  switch (type) {
    case "access_token":
